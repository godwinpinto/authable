...
public class SystemMasterAdapter implements SystemMasterSPI {
    ...
    public Mono<Long> updateDisable(String systemId, LocalDateTime lockedDateTime, String status) {
        return systemMasterRepository.updateDisable(StringUtils.rightPad(systemId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR), lockedDateTime, status);
    }

    @Override
    public Mono<SystemMasterDto> findById(String systemId) {
        return systemMasterRepository.findById(StringUtils.rightPad(systemId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR)).
