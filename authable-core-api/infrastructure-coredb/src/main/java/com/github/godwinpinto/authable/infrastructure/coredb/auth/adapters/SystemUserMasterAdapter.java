...
public class SystemUserMasterAdapter implements SystemUserMasterSPI {
    ...
    public Mono<Long> updateLoginSuccess(String accessId, LocalDateTime lastLoginDateTime) {
        return systemUserMasterRepository.updateLoginSuccess(StringUtils.rightPad(accessId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR), lastLoginDateTime);
    }

    @Override
    public Mono<Long> updateDisable(String accessId, LocalDateTime lockedDateTime, String status) {
        return systemUserMasterRepository.updateDisable(StringUtils.rightPad(accessId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR), lockedDateTime, status);
    }

    @Override
    public Mono<SystemUserMasterDto> findById(String accessId) {
        return systemUserMasterRepository.findById(StringUtils.rightPad(accessId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR))
                .flatMap(entity -> Mono.just(SystemUserMasterMapper.INSTANCE.systemUserMasterToDto(entity)));
    }

    @Override
    public Mono<SystemUserMasterDto> findBySystemUser(String systemId, String userId) {
        return systemUserMasterRepository.findBySystemUser(StringUtils.rightPad(systemId, systemUserSpacePadding, ApplicationConstants.DB_PAD_CHAR), userId)
                .flatMap(entity -> Mono.just(SystemUserMasterMapper.INSTANCE.systemUserMasterToDto(entity)));
    }
}
