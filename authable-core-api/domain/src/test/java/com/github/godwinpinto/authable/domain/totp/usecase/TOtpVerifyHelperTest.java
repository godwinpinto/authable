...
class TOtpVerifyHelperTest {
    ...
    void isUserNotActive_NotActive_Test() {
        ...
        TOtpUserMasterDto user = TOtpUserMasterDto.builder()
                ...
                .build();
        StepVerifier.create(tOtpVerifyHelper.isUserNotActive(user))
                .expectErrorMatches(e -> e instanceof NonFatalException && e.getMessage()
                        .equals("Access is inactive or disabled for the user"))
                .verify();
    }
    ...
    void verifyAndUpdateOtp_Flow_Test() {
        ...
        TOtpUserMasterDto user = TOtpUserMasterDto.builder()
                ...
                .userId("1234")
                .build();
        doReturn(true).when(tOtpCryptoSPI)
                .verify(any(), any(), any());

        doReturn(Mono.justOrEmpty(1L)).when(tOtpUserMasterSPI)
        ...
        StepVerifier.create(tOtpVerifyHelper.verifyAndUpdateOtp("123456", "TEST_SYSTEM", user))
                ...
                .verifyComplete();
        doReturn(Mono.justOrEmpty(0L)).when(tOtpUserMasterSPI)
                .updateLoginSuccess(any(), any(LocalDateTime.class));

        StepVerifier.create(tOtpVerifyHelper.verifyAndUpdateOtp("123456", "TEST_SYSTEM", user))
                .expectErrorMatches(e -> e instanceof NonFatalException && e.getMessage()
                        .equals("Error in updating database"))
                .verify();

        doReturn(false).when(tOtpCryptoSPI)
                .verify(any(), any(), any());
        ...
        StepVerifier.create(tOtpVerifyHelper.verifyAndUpdateOtp("123456", "TEST_SYSTEM", user))
                .expectErrorMatches(e -> e instanceof NonFatalException && e.getMessage()
                        .equals("Invalid OTP. Please try again."))
                .verify();
