...
public class TOtpVerifyHelper {
    ...
    TOtpVerifyHelper(TOtpUserMasterSPI tOtpUserMasterSPI,
            TOtpCryptoSPI tOtpCryptoSPI) {
    ...
    public Mono<TOtpVerifyDto> formatNoSubscriptionMessage() {
        return Mono.just(TOtpVerifyDto.builder()
                .statusCode("300")
    ...
    public Mono<TOtpVerifyDto> verifyAndUpdateOtp(String otp, String userSystemId, TOtpUserMasterDto user) {
    ...
    private Mono<TOtpVerifyDto> processSuccessAttempt(String userSystemId) {
        return tOtpUserMasterSPI.updateLoginSuccess(userSystemId,
    ...
    private Mono<TOtpVerifyDto> processInvalidAttempt(String userSystemId, TOtpUserMasterDto user) {
        ...
        return tOtpUserMasterSPI.updateInvalidAttempt(userSystemId, user.getNoOfAttempts(),
                        DateTimeUtils.getCurrentLocalDateTime())
                .flatMap(x -> disableIfAttemptsExceeded(userSystemId, user));
    }

    private Mono<TOtpVerifyDto> disableIfAttemptsExceeded(String userSystemId, TOtpUserMasterDto user) {
        if (user.getNoOfAttempts() >= maxFailedAttempts) {
            return tOtpUserMasterSPI.updateDisable(userSystemId,
                            DateTimeUtils.getCurrentLocalDateTime(),
