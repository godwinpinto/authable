...
public class TOtpCreateSecretHelper {
    ...
    TOtpCreateSecretHelper(TOtpUserMasterSPI tOtpUserMasterSPI, TOtpCryptoSPI tOtpCryptoSPI, FetchPrincipalComponent fetchPrincipalComponent) {
    ...
    Mono<TOtpUserMasterDto> isAllowedToReset(TOtpUserMasterDto user) {
        if (user.getStatus()
            ...
            return Mono.error(new NonFatalException("300", "You already have an active TOTP. " +
    ...
    public Mono<TOtpCreateNewDto> updateDbToReset(String userSystemId, TOtpUserMasterDto user) {
        ...
        return tOtpUserMasterSPI.updateEntity(user)
                .flatMap(updateCount -> {
                    if (updateCount == 1) {
    ...
    private Mono<TOtpCreateNewDto> formatSuccessResponse(String userSystemId, TOtpUserMasterDto user1) {
        return Mono.just(TOtpCreateNewDto.builder()
                .statusCode("200")
                .statusDescription("TOTP generated successfully.")
                .secretKey(tOtpCryptoSPI.getPlainTextSecret(userSystemId,
                        user1.getUserSecret()))
                .build());
    }

    private Mono<TOtpUserMasterDto> createUserDto(String systemId, String userSystemId, UserDto userDto) {
        String secret = tOtpCryptoSPI.generateSecretKey(userSystemId);
        TOtpUserMasterDto user1 = TOtpUserMasterDto.builder()
                .userSecret(secret)
                .userId(userSystemId)
                .accessType("B")
                .systemId(systemId)
